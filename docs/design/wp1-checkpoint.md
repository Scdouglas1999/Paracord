...
| `client/src/lib/attention/roomLight.ts` | Voice and text rooms → light. **Carries the exact definition of "reading"** (§3 below). |
| `client/src/lib/attention/buildingLight.ts` | Window map, counts, brightest-first ordering, the "Around now" sentence. |
| `client/src/lib/attention/litHistory.ts` | What this client has itself observed: when a room lit up, when it was last lit. |
...
| `client/src/hooks/useRoomThumbnail.ts` | Wires one room's thumbnail to the tap. |
...
**`speaking` only survives when their lights are on and they are in a room.** A
...
- `order` is the channel's own `position`. Equally-lit rooms keep the order the
  building's people gave them rather than falling back to the alphabet.
...
### `BuildingLight` — a building is a window map
...
- **Window order** (§3): voice rooms first (lit before dark, more people before
  fewer), then text rooms by reader count; ties on `order` then name.
- **At most `MAX_WINDOWS` = 16 windows.** Rooms past the sixteenth do not get a
  cell — they collapse into `overflowCount` and the caption. A window map that
  scrolls is a chart, not a building.
- **`brightness`** ranks talking > rooms lit > reading > lights on, so
  `orderBuildingsByBrightness()` puts the loudest building first (§7.1, §7.5).
  The scale is arbitrary; the *order* is the contract and the tests assert it.
- **`caption`** is "2 rooms lit · 3 reading" / "1 reading" / "Dark · nobody in".
- **`brightestRoom`** is the lit voice room whose thumbnail the building should
...
"away" clause naming people who are **not** in any room. Names inside a clause
...
app open somewhere" is not "is in this room".
...
- **A silent lurker is not counted.** The count is "people the room can tell are
...
## 4. Room thumbnails: what is live and what is a still
...
|---|---|---|
| Room you are **in**, publisher's frames land in a DOM canvas | **live, ≤ 2 fps** | `null` |
| Room you are in, platform composites **below** the webview (Linux GTK underlay) | still + LIVE dot | `native-surface` |
| Room you are in, nobody is publishing video | still | `no-publisher` |
| Room you are **not** in | still + LIVE dot | `not-joined` |
...
- A room you have not joined has **no decoder running for it anywhere on this
...
handed to it for a still room is ignored (asserted in the tests).
...
Neither the call start time nor "when did this room last empty" is on the wire.
...
- `litSinceMs` is "since this client first saw the room lit", so a duration is a
  lower bound on a call that was already running — never an over-claim.
- `lastLitMs` is `null` until this client has actually seen the room lit, so an
  unseen room reads "never lit", not a fabricated hour.
- Capped at 2 000 rooms, least-recently-touched evicted; losing an entry costs
...
| `useRoomLights(guildId)` / `useRoomLight(guildId, channelId)` | its rooms / one room |
| `useBuildingPeople(guildId)` | the people the building's rooms know about |
| `useBuildingLights()` | every building on every connected server, brightest first |
...
| `useOnAir()` | the on-air pill's model, or null when you are not in a room |
...
2. **Memoized on the narrowest slice.** `useLightSources` subscribes once and
   ...
   per-guild hook — so a building can never look different on Home than it does
   in the sidebar.
...
| `WindowMap` | `windows`, `overflowCount`, `caption`, `scale` (`sidebar` 10×13 / `home` 12×16), `columns` (capped at 8) | cells are `pc-window` + `is-talking` / `is-reading`; one `sr-only` sentence ("2 of 13 rooms lit, 4 more not shown. 2 rooms lit · 2 reading"). |
...
| `RoomThumbnail` | `room`, `height` (64 / 168 / 176), `frame`, `still`, `showOccupants`, `action` | `--radius-thumb` well; lamp anchored top-left (the reference's `radial-gradient(70% 120% at 20% 0%)`); LIVE dot + label; occupant stack; a dark room says its words rather than being a black rectangle. |
...
| `OnAirPill` | `onAir`, `onReturn` | white dot, room name, mono duration, mic state; one action. Replaces `MiniVoiceBar` in WP3. |
| `LiveDot` | `label` | 6px dot, 10.5px label — never louder than the room (§6.7). |
...
- `hooks/useLights.test.tsx` — **fixture stores**: seeded guild/channel/member/
  presence/voice/typing stores, lighting and dimming a room live.
- `hooks/useRoomThumbnail.test.tsx` — live only in the joined room on a
...
2. **The room thumbnail's lamp washed the middle of the frame.** It was sized in
   ...
   the top-left corner.
3. **The demo building had four rooms**, so the window map drew one row and the
   lamp read as a smudge over empty plate. The demo now carries thirteen rooms:
...
| Package | Use |
|---|---|
| **WP2 Buildings column** (§7.1) | `useBuildingLights()` for the per-building sections, ordered brightest-first. Per building: `SectionLabel` + `litMembersCaption(lightsOn)` ("24 in"), then `BuildingPlate` (scale `sidebar`), then rooms from `building.rooms`: a **lit voice room** is a row with `RoomThumbnail height={64}` + name + `roomCaptionFor(room)`; a **dark voice room** is a plain row with `darkRoomCaption('row')`; a **text room** is a row with a small `pc-window` dot (`is-reading` when `room.lit`) + `readingCaption(room.readingCount)`. The account plate's "Lights on" comes from `presenceLight`. |
| **WP3 Stage** (§7.2, §7.7) | `useHereNow(guildId, channelId)` → `HereNowStrip` in the header (`context="in {room}"`); `useRoomLight` for the room name, `RoomDuration` for the mono duration; `useOnAir()` + `OnAirPill` replaces `MiniVoiceBar`. Speaking rings on tiles use `pc-speaking` — `room.occupants[i].speaking` is the source. **Do not** route media through `roomFrameTap`; the Stage keeps its existing engine contracts untouched. |
| **WP4 Lobby** (§7.3) | `useBuildingLight(guildId)` for the header counts and `useAroundNow([building])` for the Around-now well (`AvatarStack` + the sentence + `lightsOnOverflowCaption`). Room cards: `RoomThumbnail height={168} showOccupants={false}` on a `Plate lit`, with the occupant `AvatarStack`, `RoomDuration` and a `variant="light"` Join in the body; dark cards use `roomCaptionFor(room, { surface: 'card', withLastLit: true })`. Text-room rows reuse the WP2 row shape. Live frames come from `useRoomThumbnail(room)` — pass `state`/`frame` straight through. |
| **WP5 Text room & DMs** (§7.4, §7.6) | `useHereNow` → `HereNowStrip` in the header ("5 reading · 19 lights on"); `LitAvatar size={36}` on timeline authors; `useRoomLight` for the inline room events ("Shop floor lit up · …" from `aroundNowSentence({ rooms: [room] })` + a `LiveDot`); composer copy from `readingCaption(room.readingCount)`. A DM's peer light is `personLight` on the recipient. |
| **WP6 Home** (§7.5) | `useBuildingLights()` (already brightest-first), `useAroundNow(buildings)` and `useLightsOnAcrossBuildings(buildings)` for the Around-now well; per building `BuildingPlate scale="home"` beside `RoomThumbnail height={176}` fed by `building.brightestRoom`; lit text rooms from `building.rooms.filter(r => r.kind === 'text' && r.lit)`. |
...
- **Cross-server identity.** `useLightsOnAcrossBuildings` sums per building; the
