# WP4 — The Lobby: a building seen from the street
...
("Rooms recipes") — the behaviour is kept, the presentation is replaced.
...
| File | Role |
|---|---|
| `client/src/components/rooms/lobby/Lobby.tsx` | The whole surface. The only place this package reads stores and decides what belongs to the building. |
| `client/src/components/rooms/lobby/LobbyHeader.tsx` | Building mark, Gabarito name, the one-line summary, Invite / settings. |
...
| `client/src/components/rooms/lobby/lobbyTime.ts` | The wall clock: event times, the day tile, a room's last stamp. |
...
**Text rooms.** Rows, never cards (§6.8): window dot (amber when somebody is
reading), name, "Priya · 10:02", the preview, the reader stack, "5 reading", and
a mention chip. Two columns at 1440, one narrower. The active room is raised and
...
nothing above the rows, so they are ordered lit-first then by the building's own
channel order.
...
timelines this client has loaded for this building's text rooms. No new endpoint,
...
media strip until the reader has opened one of its rooms.** That is the same
...
room, return to the lobby — rather than faking a store.
...
The byline, stamp and preview come from the loaded timeline. For a room this
...
1. **`SpaceBriefing` is gone, and with it the only member-facing render of a
   space's Hub settings** (welcome copy, banner, featured rooms). §7.3 has no
...
2. **The card duration is the mono `RoomDuration` ("34:12"), not the reference's
   ...
   client only counts from the moment it first saw the room lit.

A third, smaller one: `RoomThumbnail` puts a dark room's words at the **top
...
rooms" is the landmark directly below it and would otherwise make the old
locator ambiguous — and `Text channels` became `Text rooms`. Those same
landmarks, the space-settings entry and the keyboard-reachable room rows are
...
- `pages/GuildHomePage.test.tsx` — the Lobby end to end against the **real**
  ...
  strip being omitted, and a text room lighting amber when somebody types.
...
Four frames — `lobby-{lit,dark}-{1440x900,390x844}` — a **lit** building (a live
room with a screen share, people reading a text room, an event on the calendar,
an image in the strip) and the **same building all dark**.

Getting a lit building into a mocked capture needed one new piece: light comes
...
building's voice states and presences, and keeps a typing signal alive so a text
room stays amber. No app code knows about it, and it is confined to the `wp4`
branch of the spec.
...
2. **The header ellipsised the building's name on a phone** ("Kestrel Rob…").
...
3. **"+1 lights on" sat next to "Nobody's lights are on right now"** in an
   all-dark building where the reader's own lights were on. Hence the corrected
   empty sentence in §2.
4. **A text room's stamp read "Dec 31".** The row was resolving the time from
...
space's Hub welcome copy, banner and featured rooms are configured but never
