...
INSERT INTO members_new (user_id, nick, avatar_hash, joined_at, deaf, mute, communication_disabled_until)
...
INSERT INTO member_roles_new (user_id, role_id)
...
INSERT INTO bans_new (user_id, reason, banned_by, created_at)
...
-- PostgreSQL enforces FK dependencies strictly. Detach any FK that points to
-- the legacy `channels` table so we can swap in `channels_new` without
-- cascading deletes into messages/read states/etc.
DO $$
DECLARE fk RECORD;
BEGIN
    FOR fk IN
...
-- Reattach foreign keys from legacy tables that were preserved.
ALTER TABLE channel_overwrites
    ADD CONSTRAINT channel_overwrites_channel_id_fkey
...
ALTER TABLE dm_recipients
    ADD CONSTRAINT dm_recipients_channel_id_fkey
    FOREIGN KEY (channel_id) REFERENCES channels(id) ON DELETE CASCADE;
