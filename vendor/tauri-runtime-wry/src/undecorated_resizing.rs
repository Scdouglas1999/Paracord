...
mod gtk {
  ...
  pub fn attach_resize_handler(webview: &wry::WebView) {
    ...
    // Walk the parent chain to the ancestor GtkWindow. Paracord's Linux native
    // render host reparents the webview under gtk::Overlay / gtk::Fixed, so the
    // stock two-level parent().parent() assumption (GtkBox → GtkWindow) no
    // longer holds and panics on downcast.
    fn ancestor_window(widget: &webkit2gtk::WebView) -> Option<gtk::Window> {
    ...
    webview.connect_button_press_event(
      move |webview: &webkit2gtk::WebView, event: &gtk::gdk::EventButton| {
        if event.button() == 1 {
          if let Some(window) = ancestor_window(webview) {
