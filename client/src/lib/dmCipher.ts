...
import type { assertPinnedDmPeerIdentity } from './keyVerification';
import type { keysApi } from '../api/keys';

// Legacy v1 ciphertext remains readable; new sends require Signal v2.

const DM_E2EE_V1 = 1;
const DM_E2EE_V2 = 2;
const AES_GCM_NONCE_BYTES = 12;
const DM_E2EE_CONTEXT_PREFIX = 'paracord:dm-e2ee:v1:';
let hasWarnedLegacyDecryption = false;

function warnLegacyDecryption(path: string): void {
  if (hasWarnedLegacyDecryption) {
    return;
  }
  hasWarnedLegacyDecryption = true;
  console.warn(
    `[dm-e2ee] Legacy V1 ciphertext was read via ${path}. V1 is deprecated and will be removed; migrate conversations to Signal V2 sessions.`,
  );
}
...
export interface DmCipherDependencies {
  loadSession: typeof loadSession;
  saveSession: typeof saveSession;
  loadPrekeyStore: typeof loadPrekeyStore;
  savePrekeyStore: typeof savePrekeyStore;
  assertPinnedDmPeerIdentity: typeof assertPinnedDmPeerIdentity;
  keysApi: Pick<typeof keysApi, 'getBundle'>;
}

/** Bind crypto to a single storage transaction and server client. */
export function createDmCipher(dependencies: DmCipherDependencies) {
  const { loadSession, saveSession, loadPrekeyStore, savePrekeyStore, assertPinnedDmPeerIdentity, keysApi } = dependencies;
  ...
  function normalizeIdentityHex(value: string): string {
    ...
    if (!/^[0-9a-f]{64}$/.test(normalized)) {
      throw new DmE2eeError('PEER_IDENTITY_MISMATCH', 'The peer identity must be a 32-byte hexadecimal public key.');
    }
    return normalized;
  }
  ...
  async function fetchPeerBundle(
    ...
    // Keep identity mismatch distinct from missing or malformed peer prekeys.
   ...
   */
  async function encryptDmMessage(
    channelId: string,
    plaintext: string,
    myPrivateKeyEd25519: Uint8Array,
    peerPublicKeyEd25519Hex: string,
  ): Promise<MessageE2eePayload> {
    peerPublicKeyEd25519Hex = normalizeIdentityHex(peerPublicKeyEd25519Hex);
    const myPubHex = getMyPublicKeyHex(myPrivateKeyEd25519);
    await assertPinnedDmPeerIdentity(channelId, peerPublicKeyEd25519Hex);
   ...
   */
  async function encryptDmMessageV2(
    channelId: string,
    plaintext: string,
    myPrivateKeyEd25519: Uint8Array,
    peerPublicKeyEd25519Hex: string,
    ...
    peerPublicKeyEd25519Hex = expectedPeerHex;
    await assertPinnedDmPeerIdentity(channelId, expectedPeerHex, peerUserId);
    ...
    if (!session) {
      ...
      const bundle = await fetchPeerBundle(channelId, peerUserId, expectedPeerHex);
  ...
  async function decryptViaX3dhResponder(
    ...
    const signedPrekey = getSignedPrekeyPair(prekeyStore, header.spk_id);
  ...
  async function decryptDmMessage(
    ...
    peerPublicKeyEd25519Hex: string,
    peerUserId?: string | null,
    sessionMode: 'initialize' | 'existing' = 'initialize',
  ): Promise<string> {
    peerPublicKeyEd25519Hex = normalizeIdentityHex(peerPublicKeyEd25519Hex);
    await assertPinnedDmPeerIdentity(channelId, peerPublicKeyEd25519Hex, peerUserId);
    if (payload.version === DM_E2EE_V1) {
      warnLegacyDecryption('decryptDmMessage');
    ...
    if (session) {
      try {
        ...
        if (!isX3dhInitial || sessionMode === 'existing') throw err;
        try {
  ...
  /** Authenticate against this exact ratchet without replacing it through X3DH. */
  function decryptDmMessageWithSession(
    channelId: string,
    payload: MessageE2eePayload,
    ...
    return decryptDmMessage(channelId, payload, privateKey, peerPublicKey, peerUserId, 'existing');
  }

  return { encryptDmMessage, encryptDmMessageV2, decryptDmMessage, decryptDmMessageWithSession };
}
