...
export function MessagingRecoveryNotice({ runtime, encryptedConversation, channelId }: { runtime: AccountMessagingRuntime; encryptedConversation: boolean; channelId: string }) {
  ...
  return <section aria-label="Messaging recovery" className="min-w-0 rounded-[var(--radius-well)] bg-bg-raised p-3 shadow-[var(--shadow-raised)]">
    <p className="pc-display text-name text-text-primary">{review ? 'Review messages from the previous instance history' : failed ? 'Encrypted draft storage is unavailable' : reenroll ? 'Set up encryption keys on this device' : encryption ? 'Encryption needs recovery' : 'Waiting for this instance’s authenticated connection'}</p>
    <p className="mt-1 break-words text-body text-text-body">{review
      ...
      : (encryption ? state.encryptionError : state.error) ?? 'Your first drafts stay on this device until the instance confirms its history.'}</p>
    ...
    {legacy && <p className="mt-2 break-words text-meta text-text-faint">Older keys are retained because their instance ownership cannot be verified. Starting new encryption does not recover historical messages. Restore the original account backup if you need those keys; continue only to start a new session for this account.</p>}
