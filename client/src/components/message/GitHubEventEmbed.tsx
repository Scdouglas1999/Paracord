...
// Map each GitHub event category onto a semantic design token rather than a
// one-off hex, so the badge palette stays inside the system's meaning-colors.
const EVENT_COLORS: Record<GitHubEventType, string> = {
  push: 'var(--accent-info)',
  pull_request: 'var(--status-streaming)',
  issues: 'var(--accent-success)',
  issue_comment: 'var(--accent-info)',
  create: 'var(--accent-success)',
  delete: 'var(--accent-danger)',
  star: 'var(--accent-warning)',
    ...
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1.5 px-3.5 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className="rounded-xs px-1.5 py-0.5 text-section uppercase"
            style={{ backgroundColor: `color-mix(in srgb, ${color} 16%, transparent)`, color }}
          >
            {label}
          </span>
          {eventInfo.repo && (
            <span className="truncate text-meta font-semibold text-text-secondary">
              {eventInfo.repo}
            </span>
          )}
        </div>
        ...
        {safeViewUrl && (
          <a
            ...
            className="inline-flex items-center gap-1 self-start text-meta font-medium text-text-link hover:underline"
          >
            View on GitHub
          </a>
        )}
