 ...
 *   18:00 – 04:59  Evening   (the small hours belong to the night before)
 *
...
export function timeOfDayWordForHour(hour: number): TimeOfDayWord {
  ...
  if (h >= NIGHT_STARTS_AT_HOUR || h < MORNING_STARTS_AT_HOUR) return 'Evening';
...
/** Keep the person's chosen name intact; do not guess a first name. */
export function homeGreeting(now: Date, displayName?: string | null): string {
  const name = displayName?.trim();
  const greeting = timeOfDayWord(now);
