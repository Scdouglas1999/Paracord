...
export function useAvailableAccountScopes() {
  ...
  return useMemo(() => {
    // `__local__` is an account only while no entry stands for the home server;
    // otherwise it is a second name for one of the entries below.
    const scopes: AccountScope[] = user && token && !findHomeServerEntry(servers, token)
      ? [{ serverId: LOCAL_SERVER_ID, userId: user.id }]
      : [];
