...
beforeEach(() => {
  ...
  for (const scope of [a, b]) useGuildStore.getState().addGuild({ id: '1', name: `Server ${scope.serverId}`, owner_id: scope.serverId === 'a' ? '42' : 'other' } as Guild, scope);
});
describe('scoped guild view selectors', () => {
  it('immediately switches names and ownership for colliding guild IDs', () => {
    ...
    expect(result.current).toMatchObject({ name: 'Server a', owner_id: '42', scope: a });
    act(() => useServerListStore.getState().setActive('b'));
    expect(result.current).toMatchObject({ name: 'Server b', owner_id: 'other', scope: b });
  });
  it('keeps current-account lists distinct while cross-server views retain both entries', () => {
    ...
    expect(result.current.current.map(guild => guild.name)).toEqual(['Server a']);
    expect(new Set(result.current.all.map(guild => guild.key)).size).toBe(2);
    act(() => useServerListStore.getState().updateToken('b', ''));
    expect(result.current.all.map(guild => guild.name)).toEqual(['Server a']);
  });
...
describe('one server is one account', () => {
  ...
  const building = { id: 'g1', name: 'Test Server', owner_id: owner.id } as Guild;
  ...
  it('lists a server once when the added server is also the home session', () => {
