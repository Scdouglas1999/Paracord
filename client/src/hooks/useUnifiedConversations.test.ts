...
function seedChannels(input: { guildChannels?: Record<string, Channel[]>; directChannels?: Record<string, Channel[]>; loadAllDmChannels?: () => Promise<void> }) {
  if (input.loadAllDmChannels) { useChannelStore.getState().reset(); useChannelStore.setState({ loadAllDmChannels: input.loadAllDmChannels }); }
  for (const [guildId, channels] of Object.entries(input.guildChannels ?? {})) {
    const guild = useGuildStore.getState().guilds.find(guild => guild.id === guildId);
    if (!guild) throw new Error(`Missing guild fixture ${guildId}`);
    useChannelStore.getState().setChannels(guildId, channels.map(channel => ({ ...channel, guild_id: guildId })), guild.scope);
  }
  for (const [serverId, channels] of Object.entries(input.directChannels ?? {})) {
    useChannelStore.getState().setChannels('', channels, { serverId, userId: 'viewer' });
  }
}
...
function resetStores(): void {
  ...
  useReadStateStore.setState({ byAccount: {}, refreshAll: async () => {} });
...
function user(id: string, username: string): User & Relationship['user'] {
  return {
    id, username, discriminator: 0, bot: false, system: false, flags: 0, created_at: '',
    avatar_hash: null, display_name: null,
  };
}
...
describe('useUnifiedConversations — cross-server merge', () => {
  it('keeps two servers minting the same channelId collision-safe via composite keys', () => {
    ...
    expect(keys).toEqual([JSON.stringify(['a', 'viewer', '100']), JSON.stringify(['b', 'viewer', '100'])]);
    expect(needsYou.find((e) => e.key === JSON.stringify(['a', 'viewer', '100']))?.serverId).toBe('a');
    expect(needsYou.find((e) => e.key === JSON.stringify(['b', 'viewer', '100']))?.serverId).toBe('b');
  });
});
...
describe('useUnifiedConversations — needs-you scoring', () => {
  it('caps needs-you at 6 and orders by tier (mention > dm > thread > plain > voice)', () => {
    ...
    useReadStateStore.setState({ byAccount: { [JSON.stringify(['a', 'viewer'])]: { m: rs('m', '1', 3) } } });
  ...
  it('does not admit a fully-read channel into needs-you on recency alone', () => {
    ...
    useReadStateStore.setState({ byAccount: { [JSON.stringify(['a', 'viewer'])]: { read: rs('read', '500', 0) } } });
...
describe('useUnifiedConversations — pinned', () => {
  it('floats pinned entries out (in pin order) before scoring, even mentions', () => {
    ...
    useReadStateStore.setState({ byAccount: { [JSON.stringify(['a', 'viewer'])]: { hot: rs('hot', '1', 5) } } });
    // Pin order: pin2 before pin1, and pin the mention channel too.
    usePinnedStore.setState({ pinnedKeys: [JSON.stringify(['a', 'viewer', 'pin2']), JSON.stringify(['a', 'viewer', 'pin1']), JSON.stringify(['a', 'viewer', 'hot'])] });

    const { needsYou, pinned, recent } = run();
    expect(pinned.map((e) => e.key)).toEqual([JSON.stringify(['a', 'viewer', 'pin2']), JSON.stringify(['a', 'viewer', 'pin1']), JSON.stringify(['a', 'viewer', 'hot'])]);
...
describe('useUnifiedConversations — recent ordering', () => {
  it('sorts recent by last activity descending', () => {
    ...
    useReadStateStore.setState({
      byAccount: {
        [JSON.stringify(['a', 'viewer'])]: { old: rs('old', oldId), new: rs('new', newId), mid: rs('mid', midId) },
      },
    });
    const { needsYou, recent } = run();
    expect(needsYou).toHaveLength(0);
...
describe('useUnifiedConversations — graceful degradation', () => {
  it('renders rows for a server that has no read-state yet', () => {
    ...
    useReadStateStore.setState({ byAccount: { [JSON.stringify(['a', 'viewer'])]: { a1: rs('a1', '999', 0) } } });

    const result = run();
    // a1 is read (no signal) → recent; b1 has no read-state → treated as unread.
    expect(result.needsYou.map((e) => e.key)).toEqual([JSON.stringify(['b', 'viewer', 'b1'])]);
    expect(result.recent.map((e) => e.key)).toEqual([JSON.stringify(['a', 'viewer', 'a1'])]);
  });
  ...
  it('maps joined guilds to buildings with resolved serverIds', () => {
    ...
    expect(spaces).toEqual([
      { id: 'ga', name: 'Alpha', icon: null, serverId: 'a', scope: { serverId: 'a', userId: 'viewer' }, key: JSON.stringify(['a', 'viewer', 'ga']) },
      { id: 'gb', name: 'Beta', icon: null, serverId: 'b', scope: { serverId: 'b', userId: 'viewer' }, key: JSON.stringify(['b', 'viewer', 'gb']) },
    ]);
  });
});

it('hides old account pins and refreshes when the same server verifies a new account', () => {
  seedChannels({ directChannels: { a: [chan({ id: '100', type: ChannelType.DM, name: 'Old conversation' })] } });
  usePinnedStore.getState().pin({ scope: { serverId: 'a', userId: 'viewer' }, id: '100' });
  let refreshes = 0;
  useReadStateStore.setState({ refreshAll: async () => { refreshes++; } });
  const view = renderHook(() => useUnifiedConversations());
  expect(view.result.current.pinned.map(entry => entry.title)).toEqual(['Old conversation']);
  act(() => {
    useServerListStore.setState(state => ({ servers: state.servers.map(server => server.id === 'a' ? { ...server, userId: 'next', user: user('next', 'Next') } : server) }));
    useChannelStore.getState().setChannels('', [chan({ id: '100', type: ChannelType.DM, name: 'New conversation' })], { serverId: 'a', userId: 'next' });
  });
  expect(view.result.current.pinned).toEqual([]);
  expect(view.result.current.recent.map(entry => entry.title)).toEqual(['New conversation']);
  expect(view.result.current.recent[0].scope).toEqual({ serverId: 'a', userId: 'next' });
  expect(refreshes).toBe(2);
});

it('refreshes on gateway reconnect even when the verified account is unchanged', () => {
  let refreshes = 0;
  useReadStateStore.setState({ refreshAll: async () => { refreshes++; } });
  renderHook(() => useUnifiedConversations());
  act(() => { useServerListStore.setState(state => ({ servers: state.servers.map(server => ({ ...server, connected: false })) })); });
  act(() => { useServerListStore.setState(state => ({ servers: state.servers.map(server => ({ ...server, connected: true })) })); });
